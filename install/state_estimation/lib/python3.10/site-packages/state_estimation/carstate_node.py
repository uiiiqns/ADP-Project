...
class Carstate(Node):
    def __init__(self):
        ...
        # PF pose 토픽명과 child 프레임을 파라미터화한다
        ...
        # PF pose 구독자 추가
        self.pf_pose_sub = self.create_subscription(PoseStamped, self.pf_pose_topic, self.pf_pose_cb, 10)
   ...
   #     return trans
    ...
    # PF pose 콜백 정의
    def pf_pose_cb(self, msg: PoseStamped):
        self.pf_pose = msg

    # 대기 로직에서 PF pose도 기다리기
    def wait_for_messages(self, frenet_bool: bool = False):
        ...
        while self.ekf_odom is None or self.pf_pose is None or (frenet_bool and self.gb_wpnts is None):
            rclpy.spin_once(self)
            if self.ekf_odom is not None and not ekf_print:
                self.get_logger().info('Received Odometry message.')
                ekf_print = True
            if self.pf_pose is not None and not pf_print:
                self.get_logger().info('Received PF Pose message')
                pf_print = True
    ...
    def cartesian_state_loop(self):
        # publish SLAM cartesian positional and velocity data -> X
        # publish PF PostStamped and velocity data(수정 후)

        # 데이터가 둘 다 준비돼 있지 않으면 리턴
        if self.ekf_odom is None or self.pf_pose is None:
            return

        # Pose: PF PoseStamped 사용
        carstate_pose_msg = PoseStamped()
        carstate_pose_msg.header = self.pf_pose.header
        carstate_pose_msg.pose = self.pf_pose.pose

        # Odom 구성
        carstate_odom_msg = Odometry()
        carstate_odom_msg.header = carstate_pose_msg.header
        carstate_odom_msg.child_frame_id = self.base_link_frame
        carstate_odom_msg.pose.pose =carstate_pose_msg.pose

        # velocity: EKF twist 사용
