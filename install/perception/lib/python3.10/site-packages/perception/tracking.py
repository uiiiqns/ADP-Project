...
class StaticDynamic(Node):
    ...
    def __init__(self):
        ...
        # Opponent_state.ttl = 70
        # Opponent_state.ratio_to_glob_path = 0.6
        Opponent_state.ratio_to_glob_path = 0.3

        ObstacleSD.ttl = 3
        # ObstacleSD.ttl = 5
        ObstacleSD.min_nb_meas = 10
        # ObstacleSD.min_nb_meas = 10
        

        ObstacleSD.min_std = 0.16
        # ObstacleSD.min_std = 0.24

        
        ObstacleSD.max_std = 0.22
        # ObstacleSD.max_std = 0.3

        # self.dist_deletion = 7
        self.dist_deletion = 6
        # self.dist_infront = 8
        self.dist_infront = 7
        self.vs_reset = 0.1
        ...
        # self.aggro_multiplier = 2.5
        self.debug_mode = False

        # 정적장애물에 대한 로직이 추가로 필요할 때 true로 변경 (일단은 동적 장애물에 대한 로직만 필요하다고 판단하여 false로 설정)
    ...
    def publish_Marker(self) -> None:
        ...
        for tracked_obstacle in self.tracked_obstacles:
            ...
            # 앞이 아니면 skip
            if not tracked_obstacle.isInFront:
                continue
        ...
        if self.opponent_obstacle.isInitialised:
            ...
            # 앞에 있는 장애물만 publish 하기 위해 아래 4줄 변경
            car_s = self.car_s if self.car_s is not None else 0.0
            dist_front = normalize_s(self.opponent_obstacle.dynamic_kf.x[0] - car_s, self.track_length)
            is_in_front = (0 < dist_front < self.dist_infront)
    ...
    def publishObstacles(self):
        ...
        if self.opponent_obstacle.isInitialised:
            ...
            dist_front = normalize_s(self.opponent_obstacle.dynamic_kf.x[0] - car_s, self.track_length)
            is_in_front = (0 < dist_front < self.dist_infront)
            if is_in_front and (self.opponent_obstacle.dynamic_kf.P[0][0] < self.var_pub):
            # if self.opponent_obstacle.dynamic_kf.P[0][0] < self.var_pub:
                obs_msg = Obstacle()
