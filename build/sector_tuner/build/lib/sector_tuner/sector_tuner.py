...
class SectorTuner(Node):
    ...
    def marker_callback(self):
        ...
        n_sectors = int(self.sectors_params['n_sectors'])
        sec_markers = MarkerArray()

        for i in range(n_sectors - 1):
            # 1) 섹터 시작 인덱스 안전 처리 (이미 인덱스면 그대로, 실수면 반올림)
            s_raw = self.sectors_params[f"Sector{i}"]['start']
            ...
            marker_text.header.frame_id = "map"
            marker_text.header.stamp = self.get_clock().now().to_msg()
            marker_text.ns = "sector_labels"
            marker_text.id = i + n_sectors
            marker_text.type = Marker.TEXT_VIEW_FACING
            marker_text.action = Marker.ADD
            marker_text.text = f"Sector{i}"
