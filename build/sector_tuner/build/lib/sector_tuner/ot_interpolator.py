...
class OvertakingInterpolator(Node):
    ...
    def publish_sector_markers(self):
        if self.glb_wpnts_og is None or not self.glb_wpnts_og.wpnts:
            return

        global_waypoints_vis = [[w.x_m, w.y_m, w.s_m] for w in self.glb_wpnts_og.wpnts]
        N = len(global_waypoints_vis)
        sec_markers = MarkerArray()

        # s_m 배열 (start가 미터로 들어올 때 가장 가까운 인덱스 찾기용)
        s_m_arr = np.array([p[2] for p in global_waypoints_vis], dtype=float)

        for i in range(self.n_sectors -1):
            s_raw = self.sectors_params[f"Overtaking_sector{i}"]['start']

            # 인덱스/미터 자동 판별 → 인덱스로 변환
            if isinstance(s_raw, float) and (s_raw > N-1 or s_raw <= s_m_arr[-1]):
                s_idx = int(np.argmin(np.abs(s_m_arr - float(s_raw))))
            else:
                s_idx = int(round(s_raw))
            s_idx = max(0, min(N - 1, s_idx))  # 범위 클램프

            # === 방향 없는 점(SPHERE) ===
            marker = Marker()
            marker.header.frame_id = "map"
            marker.header.stamp = self.get_clock().now().to_msg()
            marker.ns = "ot_sector_points"
            marker.id = i
            ...
            marker.color.b = 1.0
            marker.color.a = 1.0
            marker.pose.position.x = global_waypoints_vis[s_idx][0]
            marker.pose.position.y = global_waypoints_vis[s_idx][1]
            marker.pose.position.z = 0.0
            ...
            marker_text.header.frame_id = "map"
            marker_text.header.stamp = self.get_clock().now().to_msg()
            marker_text.ns = "ot_sector_labels"
            marker_text.id = i + self.n_sectors
            marker_text.type = Marker.TEXT_VIEW_FACING
            marker_text.action = Marker.ADD
            marker_text.text = f"OvertakingSector{i}"
            marker_text.scale.z = 0.2
            marker_text.color.r = 0.1
            marker_text.color.g = 0.1
            marker_text.color.b = 1.0
            marker_text.color.a = 1.0
            marker_text.pose.position.x = global_waypoints_vis[s_idx][0]
            marker_text.pose.position.y = global_waypoints_vis[s_idx][1]
            marker_text.pose.position.z = 1.5
